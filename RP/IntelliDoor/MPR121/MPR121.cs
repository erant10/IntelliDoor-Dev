...
namespace MPR121
{
    public class MPR121Handler:IDisposable
    {
        ...
        public I2cDevice Connection { get; private set; }

        #region "Constructors"
        public MPR121Handler():this(MPR121_I2CADDR_DEFAULT, IRQ_HOOKUPPIN_DEFAULT){}

        public MPR121Handler(byte mprAddress, int mprIRQHookupPin)
        {
        ...
        public async Task<bool> OpenConnection(string i2cMasterId)
        {
            ...
            Connection = await I2cDevice.FromIdAsync(i2cMasterId, new I2cConnectionSettings(this.__i2cAddress)).AsTask().ConfigureAwait(false);
            
            // soft reset
            I2cTransferResult result = Connection.WritePartial(new byte[] { Registers.MPR121_SOFTRESET, 0x63 });
        ...
        {
            Connection.Write(new byte[] { reg, value });
        }
        ...
        private byte readRegister8(byte reg)
        {
            ...
            Connection.WriteRead(new byte[] { reg }, i2CReadBuffer);
